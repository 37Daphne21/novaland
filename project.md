...
EVE 대사가 타이핑되는 모든 현재·향후 페이지는 같은 전역 Interaction Rule을 사용한다. 대사 출력 중에는 화면 어느 곳이든 한 번 Click·Touch하면 전체 문장을 즉시 표시한다. Button·Link·입력·선택 요소 등 조작 가능한 요소를 누르면 대사 표시와 해당 요소의 원래 동작을 같은 첫 입력에서 실행한다. 조작 요소 내부의 Icon·Text·Label을 눌러도 같은 기준을 적용한다. 빈 배경처럼 별도 조작이 없는 영역의 입력만 대사 완료에 사용한다. 이 예외 처리는 공통 함수로 관리하며 Intro·MAP·관제실·게임에 동일하게 적용한다.
...
공통 EVE는 현재 표시한 대사와 같은 문장이 다시 요청되면 타이핑·등장·확대·Wave를 다시 시작하지 않고 정적으로 유지한다. 타이핑 중 같은 대사가 요청되면 전체 문장을 즉시 표시하며 기존 안내 완료 처리는 한 번만 실행한다. 다른 대사로 바뀔 때만 발화 Interaction을 시작한다.
...
- `assets/images/luna`에는 Markdown 작업 메모·생성 프롬프트·검수 기록을 만들거나 커밋하지 않는다. 별도 문서가 필요하다는 이유로 자산 폴더에 임의 추가하지 않는다.
- 배경, 인물과 복잡한 Emblem처럼 이미지가 필요한 요소만 래스터 이미지로 사용한다.
- 앞으로 제작하는 전체 화면용 래스터 배경은 가로 2560px 이상인 고해상도 원본을 확보한다. MAP·관제실·게임 등 전체 화면에 표시되는 배경에 공통 적용한다. 실제 표시 비율과 확대·크롭 범위를 고려하고, 생성 요청 문구가 아니라 최종 파일의 실제 픽셀 크기를 검사한다.
- 저해상도 결과를 단순 확대하거나 파일 크기만 변경한 이미지는 고해상도 원본 기준을 충족한 것으로 취급하지 않는다. 도구가 기준 해상도를 제공하지 못하면 한계와 미확보 상태를 먼저 알리고, 기준 미달 후보를 최종 전체 화면 배경으로 적용하지 않는다.
...
미션 목록 하단 Progress는 해금된 시설 수가 아닌 복구 완료 시설 수를 표시한다. 최초 0 / 4·0%에서 각 시설 완료 시 한 칸씩 증가하며, WONDER 해금 시 3 / 4·75%, WONDER 완료 시 4 / 4·100%가 된다. 진행 문구는 복구 완료 시설이며 전체 완료 시 전체 복구 완료로 전환한다. 접근성 값도 완료 수 기준으로 일치시킨다.
...
- 최초 시설 진입 전 다른 Mission을 선택하면 기존 지도 Card는 다시 비활성화된다.
- Mission List 선택만으로 Control Room에 진입하지 않는다.
- Mission List 선택 → 지도 시설 Card의 두 단계 안내는 전체 탐험의 첫 시도에만 필요하다. 진행 이력이 생긴 뒤에는 모든 해금 시설을 목록 선택 없이 지도에서 바로 열 수 있다. 시설마다 최초 안내를 반복하지 않는다.
- 이후 Mission List는 EVE 안내만 바꾸고 현재 진행 Mission의 선택 상태를 유지한다. 복구 완료 시 다음 Mission을 선택하며 완료 시설도 직접 재진입할 수 있다. Mission List와 지도 시설 Card에 이동 모션을 사용하지 않는다.
...
- 관제실 시작 Button은 최초 진입에서 MISSION START, 저장된 진행이 있으면 계속하기를 표시한다. 완료 시설에는 시작 Button을 표시하지 않는다.
...
MAP과 관제실의 `ui-panel`은 동일한 청회색 Glass 배경을 사용한다. 배경은 rgba(8, 21, 34, .75)의 균일한 청회색이며 Backdrop Blur와 내부의 뿌연 Glow 없이 얇은 테두리·상단 반사광만 사용한다. 헤더·미션·EVE·최근 기록·시설 현황에 동일하게 적용한다. 개별 패널·EVE 집중·모바일 미션 펼침 상태에서 배경 농도를 덮어쓰지 않는다. 모바일 헤더는 기존의 배경 없는 HUD 구조를 유지한다. 시설별 테두리·아이콘·강조색은 유지하며 게임 내부 패널은 이 변경 범위에 포함하지 않는다.

Control Room은 시설 상태를 확인하고 Mission을 시작하는 관제 공간이다. 시설마다 분위기와 비주얼은 다르지만 정보 계층, 공통 컴포넌트와 사용 흐름은 유지한다.

모든 관제실은 배경 원본의 색감·밝기를 유지한다. 배경 위에 `::before` 색상·그라데이션 레이어를 추가하지 않으며 가장자리 처리는 기존 `::after`의 Box Shadow만 사용한다. NOVA·LUNA와 이후 SPARK·WONDER에도 동일하게 적용한다. 가독성은 UI 패널에서 조정한다.
...
공통 창 제목은 미션 가이드로 유지하고 시설명을 함께 표시한다. 본문 소제목은 시설별 미션명(NOVA는 레일 복구 미션)을 16px로 표시하며, 왼쪽 연습 영역의 공통 제목은 미션 미리 해보기(TRY THE MISSION)로 사용하며 시설별 복구 방식을 제목에 넣지 않는다. 왼쪽 연습 조작에는 순번을 표시하지 않는다.

오른쪽 설명 목록의 01·02·03은 유지하며 공통 ui-panel·ui-info-list 항목을 조합한 박스로 표시한다. 번호는 16px 숫자 Font와 공통 강조색을 사용하고 Badge 테두리·배경·Padding 없이 제목 첫 줄에 정렬한다. 설명 박스는 내부 Padding 16px·박스 간격 8px를 유지하며 제목 14px·설명 13px·두 문구 간격 4px·설명 줄높이 1.6으로 표시한다. 테두리는 공통 선색의 대비를 낮추고 내용 길이에 따른 높이 차이는 허용한다. 약한 라운드(var(--radius-sm), 8px)는 부모 목록에 ui-info-list--angular를 한 번만 지정해 모든 직계 항목에 적용하며 항목별 Modifier를 반복하지 않는다. 전용 절단 모서리나 장식 테두리를 새로 만들지 않는다.

연습 안내는 현재 언어의 가장 긴 연결 안내가 차지하는 공간을 항상 확보하여 회전 상태별 줄 수 변화가 레일·안내·버튼 위치를 바꾸지 않도록 한다. 복구 시작과 게임으로 돌아가기 Button은 모두 설명 영역 가운데 정렬한다. 두 버튼의 정렬은 공통 부모 범위 선택자로 관리하며 상태별로 나누지 않는다.

Guide 팝업 높이는 콘텐츠에 따라 자동으로 결정하며 고정 높이로 빈 공간을 채우지 않는다. 화면 높이에서 외곽 여유를 뺀 max-height만 두고 초과할 때 본문을 스크롤한다. Countdown·Play의 전체 화면 높이와 구분한다. Guide 팝업 외곽 Padding은 PC 24px·작은 화면 16px로 위아래를 동일하게 유지한다. 본문은 제목 구분선 아래에만 같은 간격을 두어 외곽 하단 Padding과 중복하지 않는다. 내용이 실제 화면 높이를 넘는 작은 화면에서는 본문 스크롤을 유지한다.

빈 공간은 공간 선택 문구와 선택 전 점선 Motion으로 안내한다. 배치 후 같은 Button은 조각 제거로 바뀌며 재배치할 수 있다. 회전은 조각이 없을 때만 비활성화하고 연결 후에도 90°씩 반복할 수 있다. 잠금 Icon 대신 비활성 스타일과 안내 문구로 선행 조건을 설명한다. 모션 감소 설정에서는 점선 Motion을 끈다. 가로로 배치한 시설명과 창 제목에는 세로 제목용 아래 Margin을 적용하지 않는다.
...
점검·일시정지·실패·완료 팝업은 `ui-panel`의 공통 표면과 `ui-overlay__dialog`의 너비·Padding을 사용한다. 게임 위 배치와 화면 기준 높이 제한은 Mission 부모 범위에서 관리하며, 너비·Padding 차이는 부모의 공통 Option 변수로 전달한다. `mission-result`·`mission-testing`은 내부 콘텐츠 배치를 담당하고 외곽 규칙을 반복하지 않는다. 낮은 화면의 필요한 세로 스크롤은 유지하고, 자식 콘텐츠 너비 오류를 가로 스크롤 숨김으로 덮지 않는다. 점검 열차 영역과 이동 끝점은 실제 부모 내부 너비를 기준으로 계산한다.
...
- 게임 배경은 관제실에서 보이는 온실 내부로 들어온 공간으로 표현한다. 관제실의 청록빛 유리·금속 프레임·달빛·따뜻한 조명과 재질 품질을 이어간다. 크리스탈과 고정 받침의 접점에는 실제 발 위치에 맞춘 접촉 음영과 반사광을 두며 전체 윤곽 발광으로 접지감을 지우지 않는다.
...
- 고정 원판은 배경에 포함해 지면 접촉 그림자·식재 가림·원근·광원을 함께 표현한다. 회전하는 크리스탈과 빛길·조작 효과는 분리하며, 복구 전후 원판 위치·형태와 화면 비율 변경 시 정렬을 유지한다. 꽃과 Lotus도 같은 장면의 재질·접지 기준을 따른다. NOVA COASTER의 승인된 디자인과 동작은 유지하며 단계 수·제한 시간·기계 UI를 그대로 가져오지 않는다.
- 달은 넓은 화면에서도 보이는 안전 영역에 두고, 중앙 Lotus는 연못·줄기·수면 반사와 한 장면으로 표현한다. 현재 회전 상태에서 다음 미복구 목표로 이어지는 조작을 안내한다. 해당 목표의 실제 빛줄기 끝점에 ◇ 하나만 표시하고 다음에 돌릴 Prism 하나만 맥동한다. 한 번 빛을 받은 Prism은 빛길을 바꿔도 일정한 발광을 유지하며 재시작에서 초기화한다. Prism은 어느 방향에서든 입사한 빛을 받고, 회전으로 정한 출구 한 방향으로만 보낸다. 별도의 입구 방향 조건이나 양갈래 미리보기를 두지 않는다. 받침에도 출구 한 개만 표시한다. 유효한 도착점인 Prism·꽃 군락·조건을 채운 Lotus로 가는 빛은 실선이다. 허공이나 세 조각을 모으기 전 Lotus처럼 유효하지 않은 도착점은 점선이다. 세 군락 이후에는 Lotus만 안내하고 완료 후 ◇와 맥동을 종료한다. 완료 연출은 주변 밝기가 1초 동안 서서히 올라온 뒤, 1초 시점에 Lotus 개화·빛줄기와 출구 소멸·완료 문구 등장이 함께 시작되는 순서다. 개화는 2초, 빛줄기·출구 소멸과 문구 등장은 같은 0.8초 동안 진행한다. 주변 밝기는 고정 그라데이션의 opacity로 전환해 순간적으로 밝아지지 않도록 한다. 출구 표시·빛줄기·목표 ◇는 동일한 보드 좌표를 사용하며 Lotus만 별도 높이로 내리지 않는다. 회전은 90° 단위로 유지한다. ◇는 동일한 도착 좌표를 기준으로 은은한 상하 이동과 밝기 변화를 반복한다. Pause·문서 비표시에서 연출을 멈추며 모션 감소에서는 정적 표시를 유지한다.
- A/B/C/D·90°·군락 이름과 상태는 보드 위에 상시 표시하지 않는다. 오른쪽 아래 조작 설명도 두지 않는다. Guide·EVE와 접근성 이름으로 필요한 안내를 유지하고, 다음 크리스탈의 발광과 키보드 Focus로 조작 가능함을 알린다. 마우스 Hover 발광은 사용하지 않는다. 실제 빛 수신 상태와 구분하며 완료 후 조작 유도 발광을 종료한다. 모션 감소에서는 정적인 신호를 유지한다.

**확정된 한 화면 퍼즐 구성**

- 보드가 STEP마다 교체되는 구조 대신 한 정원 안에서 세 군락을 깨우고 중앙 Lotus를 완성한다. 이미 피어난 군락은 계속 남는다.
- Lotus 완료는 세 군락 복구와 현재 빛길의 네 Prism 연결을 모두 요구한다. 광원 → 왼쪽 위 → 왼쪽 아래 → 오른쪽 아래 → 오른쪽 위 → Lotus 경로로 도착해야 하며, 왼쪽 위에서 Lotus로 바로 보내는 경로는 복구 후에도 점선으로 표시하고 완료 처리하지 않는다. 다음 조작 안내도 전체 연결 경로를 기준으로 한다.
- 루나 EVE 패널은 대사 줄 수에 따른 실제 내용 높이로 배치하고 높이 변화만 0.24초 ease-out으로 전환한다. 고정 높이·최소 높이를 남기지 않으며 모션 감소에서는 즉시 반영한다.
- 안내 ◇는 선택된 미복구 꽃이 깨어날 때까지 같은 목적지를 유지한다. 다른 Prism을 회전해도 목적지를 바꾸지 않으며 해당 목적지로 가는 다음 Prism만 다시 계산한다. 다른 순서의 군락 복구는 허용하고 안내 목표 복구 후 다음 목적지를 선택한다. 다음 안내 Prism 외의 것을 조작했을 때 새 군락 복구·전체 완료가 없다면 EVE가 현재 맥박치는 Prism과 ◇를 안내한다.
- 단일 출구는 이미지 대신 CSS 선·원으로 표시한다. 위·아래에서도 결정에 가려지지 않도록 끝점 좌표를 조정하고 회전 중에도 원의 크기는 고정한다. 출구의 선은 기존처럼 결정 뒤에 가려지는 배치를 유지한다. 크리스탈 선명도 개선은 사용자 요청으로 보류한다. 다음 조작 Prism의 자체 발광과 점등 상태는 유지하고 마우스 hover 발광은 사용하지 않는다. 키보드 focus 표시는 유지한다.
- 단일 광원·고정 Prism 4개·꽃 군락 3곳·중앙 Lotus를 사용한다. 클릭·Enter·Space로 시계 방향 90° 회전하며, 어느 방향에서 들어온 빛이든 회전으로 지정한 단일 출구 방향으로 나간다. 빛이 되돌아오는 경로도 허용하되 순환은 감지해 계산을 종료한다.
- 같은 Prism을 다시 돌려 다른 군락으로 경로를 전환한다. 군락 방문 순서는 자유롭고 빛이 닿으면 해당 Fragment를 한 번만 회수하고 개화한다. 별도 연결 확인 버튼과 시간 제한은 두지 않는다.
...
- 정원 배경은 관제실·Prism과 이어지는 섬세한 재질을 사용한다. 유리·바닥·연못 테두리에 반복되는 둥근 알갱이와 과한 돌출·반짝임을 피한다. 세 군락은 복구 전에는 잎과 닫힌 작은 봉오리 위주로 꽃색을 억제하고, 복구 후에는 흰색·살구색·보라색의 열린 꽃잎이 확실히 드러나게 한다. 광원 구체·Lotus·Prism·배치와 게임 동작은 배경 개선을 이유로 임의 변경하지 않는다.
- 밤 테마는 현재 적용한 복구 전후 배경으로 확정하며 그대로 유지한다. 낮 테마는 후속 제작 대상으로, 사용자가 제시한 `Codex 이미지 2026년 9월 6일 오후 07_34_02.png`의 밝고 맑은 분위기를 참고한다. 현재 구도·온실·받침·꽃 군락·Lotus·Prism·광원 구체·게임 좌표와 동작을 유지하고, 온실 밖 달을 해로 바꾸는 방향으로 제작한다. 이 참고 이미지를 밤 테마의 복구 후 배경으로 교체하지 않는다.
- Prism은 두께·절단면이 보이는 결정과 윗면·옆면·밑면이 보이는 석재 받침으로 구성한다. 받침은 고정하고 결정의 방향별 이미지와 단일 출구 표시가 90° 회전을 표현한다. 발광은 결정 본체 위에 겹쳐 표시하며, 본체의 내부 무늬·절단면 선명도와 전환 중 위치 안정성을 함께 검수한다. 군락은 낮은 식재·이끼·작은 동반 꽃을 포함하며 밑동을 배경과 연결한다.
- 루나 EVE는 안내 대사가 변경될 때만 타이핑한다. 같은 대사를 반복하는 조작은 기존 표시를 유지한다. 최초 안내는 한 번 타이핑한다.
- 꽃을 찾는 동안 EVE는 다이아몬드가 있는 꽃과 맥박치는 크리스탈을 안내한다. 길 변경·이미 피어난 꽃 재방문에는 별도 상태 설명을 반복하지 않는다. 새 군락 개화 시 다음 다이아몬드를 안내하고, 세 군락 복구 후에는 네 크리스탈을 모두 거쳐 중앙 연꽃에 연결하도록 안내한다. 조기 연꽃 도달·이어하기·전체 완료는 해당 상황에 맞는 문장을 표시한다.
...
- 정원 게임 주소는 `luna-game.html`이며 MAP에서 개방된 LUNA 선택 → 공통 관제실 → MISSION START로 진입한다. 관제실 안의 전체 화면 Dialog에서 독립 게임을 열고 실제 탐험 진행·Checkpoint·Stamp에 연결한다. 언어는 기존 공통 설정을 따르며 게임 안에 별도 언어 전환 버튼을 두지 않는다.
- 관제실은 원본 구도를 유지한 2560×1441 WebP 배경 `assets/images/control-room/bg-control-room-luna.webp`와 원본 EVE를 사용한다. 이 배경은 사용자 승인에 따라 기존 원본을 확대·변환한 예외이며 신규 고해상도 디테일을 생성한 자산으로 간주하지 않는다. 온실·왼쪽 구체와 깊이 있는 관제 테이블을 유지한다. 배경을 축소하거나 테이블을 납작하게 바꾸기보다 루나 시작 버튼의 폭·패딩을 줄여 하단에 배치한다. 꽃 문양 노출을 우선하며 테이블 앞쪽 받침 일부와 버튼의 겹침은 허용한다. 배경 색상 레이어 없이 공통 `::after` Box Shadow만 사용한다. 왼쪽 제목·EVE, 오른쪽 정원 상태·복구 목표 구성을 유지하고 레일 수·STEP·운행 점검 대신 고정 Prism·세 군락·Lotus의 휴면 상태를 안내한다.
...
- 루나 MISSION START는 CSS 유리 표면·금빛 이중 테두리와 코스터와 동일한 4초 주기의 부드러운 달빛 반사를 사용한다. Hover·키보드 Focus에서 테두리와 표면을 밝히며 눌림 피드백을 제공한다. 완료 상태와 모션 감소 설정에서는 달빛 애니메이션을 멈춘다. 코스터 버튼의 기존 Frame·빛 이동·완료 표현은 유지한다. 루나 완료는 공통 진행 저장·탐험 여권 Stamp·다음 시설 해제에 반영한다.
- 루나는 복구된 정원 자체를 완료 결과로 사용한다. Lotus 개화 후 별도 결과 팝업이나 자동 이동 없이 `정원이 다시 숨 쉬어요` 문구 아래 `Passport 기록 확인` 버튼을 표시한다. 버튼을 누르면 MAP 위 Explorer Passport의 LUNA 기록 페이지에서 Stamp를 수여하고, Passport를 닫으면 SPARK ENERGY TOWER가 선택·안내된 MAP으로 돌아간다.
- 첫 진입은 Guide → 시작 → Play 순서다. 플레이 중 Guide는 진행을 유지하고 닫으면 게임으로 돌아온다. Pause는 NOVA와 동일한 공통 패널에서 계속하기 → 다시 시작 → 관제실 → MAP 순서로 제공하며 다시 시작은 정원 전체를 초기화한다. 별도 상단 복귀 바와 하단 다시 체험하기 버튼은 두지 않는다. Play의 Escape는 Pause, Pause와 최초 Guide의 Escape는 관제실, 플레이 중 Guide의 Escape는 Play로 이동한다. 브라우저 뒤로가기는 관제실로, 관제실 뒤로가기는 MAP으로 돌아간다. 진행 중 관제실·MAP으로 돌아가도 크리스탈 방향·개화 군락·다음 안내 목표를 Checkpoint에 보관한다. 관제실의 계속하기는 최초 Guide를 다시 열지 않고 저장된 정원을 복원한다. 새로고침하면 해당 관제실에서 이어하기를 제공하며, 다시 시작만 정원을 초기화한다. iframe 문서 교체는 별도 History 항목을 만들지 않는다.

**미정·보류**

- 현재 배치의 최종 난이도는 사용자 플레이 후 조정한다. 추가 색 혼합·분기와 별도 보드 확대는 현재 범위가 아니다.
- Guide는 설명·그림·독립 조작 연습·시작 버튼으로 구성한다. 실제 크리스탈 이미지를 눌러 90°씩 회전하며 다이아몬드가 있는 꽃에 빛을 연결하면 점선이 실선으로 바뀌고 꽃이 핀다. 꽃은 경로를 바꿔도 유지하며 다시 연습으로 초기화한다. 연습은 실제 게임의 회전·수집·저장 상태를 변경하지 않는다. 작은 화면에서는 내부 스크롤을 허용하고 처음 열 때 설명부터 보여준다.
- 완료된 시설의 미션 목록 재선택은 해당 시설의 현재 복구 상태만 안내한다. 완료 당시 다음 시설 해제 대사를 재사용하지 않는다. 루나 관제실의 완료 표시는 코스터와 같은 공통 구조로 정원 생명 신호 정상과 세 군락·연꽃 개화 결과를 표시한다.
- 완료 저장·Checkpoint 복원·완료 결과의 MAP/Passport 반영을 연결한다. 관제실은 저장된 군락 수와 연꽃 대기·개화 상태를 표시하며 완료 결과에 코스터 전용 레일 문구를 사용하지 않는다. MAP 복귀 대사는 현재 미완료 시설과 이어하기 여부를 반영한다. 모바일 전용 게임 HUD는 후속 범위다. 실제 기기·Safari·운영체제 모션 감소 검증도 별도 진행한다.
...
- 장르: Core 위치·방향 기억 퍼즐
- 핵심 조작: 4초 동안 공개되는 Core의 위치와 방향을 기억한 뒤 여섯 Slot에 Drag 또는 선택·배치하고 90도 단위로 회전
- 규칙: Core는 빈 Slot에만 배치하고 잘못 배치한 Core는 재이동할 수 있다. 모든 Core를 배치하면 자동 검사하며 실패 시 틀린 위치를 특정하지 않고 배열 일부가 맞지 않는다고만 안내한다. 배치 시간 제한은 없다.
- 힌트: `힌트 보기`를 누르면 정답 위치와 방향을 3초 동안 잔상으로 다시 표시한다. 횟수 제한과 불이익은 없다.
- 단계: 1단계는 세 Core를 회전 없이 배치하고, 2단계는 네 Core 중 삼각·번개, 3단계는 다섯 Core 중 삼각·번개·화살, 4단계는 여섯 Core 중 삼각·번개·화살·별의 방향까지 맞춘다. 원·육각처럼 회전해도 같은 Core는 방향 판정과 회전 조작에서 제외한다. 각 단계 완료 후 `다음 단계`로 이어진다.
- 완료: 각 단계의 위치와 방향이 일치하면 충전율이 25% → 50% → 75% → 100%로 상승하고, 4단계 뒤 발사 테스트를 실행해 미션을 완료한다.
- 난이도: 단계별 Core 수와 회전 대상 수, 여섯 Slot의 빈자리, 4초 공개 시간으로 상승한다.
...
- SPARK 관제실은 공통 Control Room 구조를 재사용하고 시설 고유 문구·상태·아이콘·Amber Theme만 분리한다. MISSION START는 간단한 3단계 Guide를 거쳐 카운트다운·정답 공개·Core 배치 화면으로 진입한다. 관제실의 추가 시각 디벨롭은 게임 화면과 전체 SPARK Flow를 완성한 뒤 진행한다.
...
- Play의 Escape는 Pause, Play에서 연 Guide의 Escape는 게임 복귀로 처리한다. 키를 길게 눌러 다음 창까지 연속으로 닫지 않는다. 초기 Guide의 닫기는 관제실 복귀다.
- Pause·Guide·화면 이탈 시 시간뿐 아니라 예약된 단계 전환도 정지한다. 복귀 시 완료한 단계에서 다음 단계로 한 번만 이동하며 이전 단계의 조각·선택 위치를 복사하지 않는다.
- Mission을 닫는 모든 경로는 Timer·Countdown·단계 전환·최종 점검·완료 자동 이동을 정리한다. 진행 중 이탈은 Checkpoint를 저장한다.
- EVE 전체 Click 즉시 노출은 현재 보이는 화면에서만 처리한다. 숨겨진 화면이나 Modal 뒤의 EVE가 앞 화면의 조작을 소비하지 않는다.
- Intro 언어 변경은 진행 중인 대사의 완료 동작을 보존한다. 발급 대사를 즉시 완료해도 MAP 진입 버튼이 정상적으로 열린다.
- Overlay는 배경을 inert로 만들고 처음 Tab·Shift+Tab부터 내부 포커스를 유지한다. 닫을 때 실제로 보이는 원래 조작 요소로 복원하며 언어 선택 재렌더링으로 포커스를 잃지 않는다.
- 검수는 Preview의 외형 확인과 일반 주소의 실제 저장·진행 검증을 분리한다. 최초 진입뿐 아니라 재진입·새로고침·뒤로가기·완료 직전 이탈·언어 변경을 확인한다.
- UI 수정은 기존 승인 형태·문구를 기준으로 비교한다. 목록은 공통 부모의 간격·색·라운드 규칙을 우선하고 항목별 장식 클래스를 반복하지 않는다. 콘텐츠 흐름으로 해결 가능한 고정 높이를 추가하지 않는다.


### 실행과 자동 검증

프로젝트 루트에서 `python -m http.server 8774 --bind 127.0.0.1`로 정적 서버를 실행한다. 사용 중인 포트라면 다른 포트를 사용한다. 별도 빌드와 패키지 설치는 필요하지 않다.

`node --test tests/regression.test.mjs`는 Node 기본 모듈만 사용한다. 실제 소스의 진행 상태·타이머·모달·Intro 완료 콜백을 격리한 DOM/저장소 대역으로 검사하며 브라우저 저장 데이터를 변경하지 않는다. 이 검사는 실제 렌더링·네이티브 Dialog·전체 사용자 흐름 검수를 대체하지 않는다. 수정 후 JavaScript 문법 검사와 `git diff --check`를 함께 실행한다.
...
| luna | play | 초기 정원 0/3 |
| luna | completed | 세 군락 복구 후 Lotus 개화와 Passport 기록 버튼 자동 재생 |

일시정지는 게임에서 여는 공통 UI를 유지하며 직접 진입 Preview는 제공하지 않는다. 완료 확인은 시설의 플레이 경험에 맞춰 구성한다. 코스터는 별도 최종점검(`testing`)에서 연결 검사·시운전·안전 점검을 수행하고, 루나는 최종 빛 연결과 Lotus 개화를 게임 장면 안의 완료 확인으로 사용해 별도 팝업으로 전환하지 않는다. 루나에는 카운트다운·시간 초과가 없으며 실제 완료 결과·Stamp·복구 후 관제실은 공통 진행 저장에 연결한다. SPARK·WONDER의 완료 확인 방식은 각 시설 플레이를 설계할 때 확정한다.

예전 시설 없는 `mission-preview`, `control-room` 호환 조합과 `luna-preview` 상태 진입을 제거했다. 공유 Preview는 위의 `index.html?facility=...&mission-preview=...`만 사용한다. 미지원 주소는 정상 Preview로 연결하지 않는다. `luna-game.html` 자체는 실제 게임을 담는 내부 문서이므로 유지한다. 루나 게임 Preview는 초기 시작(`play`)과 개화 연출(`completed`)을 제공한다. `completed`는 기존 게임 회전으로 세 군락을 복구한 뒤, 이미지 로딩과 약 3.6초의 활성 대기를 거쳐 마지막 회전을 실행해 Lotus 개화를 재생한다. 새로고침하면 재생을 반복할 수 있고 수동 Pause·비표시 중에는 대기가 멈춘다. Preview는 실제 탐험 저장과 분리한다. 군락별 복구·조건 부족의 별도 직접 진입 주소는 제공하지 않는다. 관제실·Guide 테스트 진입은 유지한다.
