...
- 단일 출구는 이미지 대신 CSS 선·원으로 표시한다. 위·아래에서도 결정에 가려지지 않도록 끝점 좌표를 조정하고 회전 중에도 원의 크기는 고정한다. 출구는 본체 위의 별도 표시로 합성하여 본체의 선명도·픽셀 위치에 영향을 주지 않도록 검수한다. 다음 조작 Prism의 자체 발광과 점등 상태는 유지하고 마우스 hover 발광은 사용하지 않는다. 키보드 focus 표시는 유지한다.
...
- 루나 MISSION START는 CSS 유리 표면·금빛 이중 테두리와 코스터와 동일한 4초 주기의 부드러운 달빛 반사를 사용한다. Hover·키보드 Focus에서 테두리와 표면을 밝히며 눌림 피드백을 제공한다. 완료 상태와 모션 감소 설정에서는 달빛 애니메이션을 멈춘다. 코스터 버튼의 기존 Frame·빛 이동·완료 표현은 유지한다. 루나 게임의 실제 완료 저장 연결은 기존 보류 범위를 따른다.
- 첫 진입은 Guide → 시작 → Play 순서다. 플레이 중 Guide는 진행을 유지하고 닫으면 게임으로 돌아온다. Pause는 NOVA와 동일한 공통 패널에서 계속하기 → 다시 시작 → 관제실 → MAP 순서로 제공하며 다시 시작은 정원 전체를 초기화한다. 별도 상단 복귀 바와 하단 다시 체험하기 버튼은 두지 않는다. Play의 Escape는 Pause, Pause와 최초 Guide의 Escape는 관제실, 플레이 중 Guide의 Escape는 Play로 이동한다. 브라우저 뒤로가기는 관제실로, 관제실 뒤로가기는 MAP으로 돌아간다. 현재 관제실로 돌아오면 샘플 조작 상태는 초기화된다. iframe 문서 교체는 별도 History 항목을 만들지 않는다.
