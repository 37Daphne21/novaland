...
Mission Preview는 게임을 완료하지 않고 동일한 실제 화면을 확인하는 용도이며, 기존 Explorer와 진행 데이터를 덮어쓰지 않는다. 복구 완료 Preview에서는 NOVA COASTER 완료·Stamp와 LUNA 해금 상태를 임시로 구성하고 Passport 기록 보기까지 연결한다.
