...
개발 참고 시안은 `references/design`에 보관한다. NOVA COASTER Control Room 2장과 Common Flow·UI·Facility·Motion·Theme Board 7장을 포함하며, 각 이미지의 사용 범위와 우선순위는 해당 폴더의 `README.md`를 따른다.
...
- 핵심 조작: 전체 Rail Network에 표시된 세 곳의 고장 구간을 선택하고 조각 선택·회전·조합으로 복구
- 규칙: 정상 구간은 유지하며 고장 구간만 조작한다. 각 구간은 연결 방향이 일치해야 복구된다.
