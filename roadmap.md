...
검증: 별도 Edge 브라우저의 격리된 테스트 탐험가로 코스터 세 단계·최종 점검·완료 기록부터 루나 진입, 1/3 관제실 왕복, 새로고침 복원, 루나 완료·다음 시설 해제까지 실제 UI를 조작했다. 1366×768·1280×480·390×844에서 Guide 복귀·관제실 이어하기·재시작·MAP 복귀와 Preview 저장 격리를 확인했다. 실제 사용자 데이터·인앱 브라우저·Safari 검증과 모바일 전용 HUD는 별도 범위다.

자동 회귀 테스트 41개를 통과했다. 1908×925에서 첫 목록 안내 전 비활성·첫 진입 후 직접 재진입·기존 진행자의 루나 직접 진입을 확인했다. 같은 목록 반복 시 선택 상태 유지, EVE 타이핑·실행 중 애니메이션 0개, 지도 카드 Hover 전후 동일 경계를 확인했다. 탐험 여권은 2/4 복구를 표시하며 브라우저 뒤로가기 후 루나 1/3 진행도와 Guide 생략도 확인했다.

### 소스 연결 지점

루나 체험 가이드와 완료 상태 표시를 적용했다. 완료 시설 목록 재선택은 다음 시설 안내 대신 해당 시설의 현재 상태를 설명한다. 가이드는 코스터와 같은 공통 헤더·2열 배치·3단계 설명·시작 버튼을 사용하고, 루나 전용 체험에서 다이아몬드·크리스탈·빛길·개화를 보여준다. 연습은 실제 게임 진행을 바꾸지 않는다. 2026-09-18 별도 Edge의 1908×925·390×844·1280×480에서 화면·클릭·키보드·다시 연습·게임 복귀를 확인했고, 낮은 화면에서 안내 제목이 가려지던 초기 스크롤을 수정했다. 자동 테스트 41개를 통과했다.
