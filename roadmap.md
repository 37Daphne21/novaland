...
이전 V3: 빈 석재 광장을 식재·좁은 돌길·작은 연못으로 바꾸고, 군락마다 주변 화단이 복구되는 배경 레이어를 연결했다. Lotus 완료에서는 밝고 산뜻한 전체 정원으로 전환한다. Prism은 사용자가 선호한 기존 가늘고 비대칭인 결정과 아이보리 받침을 다시 사용한다. 신규 광학 결정·회색 받침과 어두운 복구 배경은 적용하지 않는다. 사용 자산·생성 프롬프트는 `assets/images/luna/visual-v3-notes.md`에 기록한다.
