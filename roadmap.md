...
`luna-sample.html`에 Prism 4개·군락 3곳·Lotus의 독립 퍼즐을 구현했다. 입체 받침·결정 4방향 시트·군락 2상태 시트·Lotus 2상태 시트를 추가하고 배경과 EVE 원본은 유지했다. 세 조각 이전 Lotus 연결은 완료되지 않으며 완료 후 회전 잠금과 Pause 안의 다시 시작을 제공한다. MAP → 루나 관제실 → 정원 게임 진입과 관제실·MAP 복귀를 연결했다. 현재 게임은 독립 샘플을 Dialog 안에서 열며 탐험 저장·완료 기록은 연결하지 않았다.
