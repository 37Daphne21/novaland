...
이미지 정리: 미사용 구버전·후보 29개(61,693,706바이트)를 삭제하고 사용 자산 9개의 버전·임시 접미사를 역할 이름으로 정리했다. 참조 경로를 함께 변경했으며 파일 해시는 동일하다. 보관용 관제실·게임 레퍼런스 2개도 사용자 요청으로 삭제했다. 관제실은 원본 구도·색감의 2560×1441 WebP로 확정하고 테이블 축소안은 제외했다. 1908×925 브라우저에서 적용 화면을 확인했다. 새 이미지 문서는 만들지 않았다. 정적 이미지 경로 누락 0, 회귀 테스트 35개 통과, 코스터·루나 관제실/게임 Preview에서 HTTP·스크립트 오류 0 및 루나 직접 완료를 확인했다. 이 정리 내용은 관제실 배경과 함께 커밋한다.

루나 게임 화면은 현재 상태로 마무리한다. 크리스탈 선명도는 사용자 요청으로 보류한다. 다음 작업은 코스터 → 루나 전체 흐름, MAP 안내·목표·현황, 루나 관제실 왕복 이어하기 검수와 수정이다. 다른 Prism 회전 차단 제안은 미승인·미적용이다.

EVE 내용 높이 전환을 0.24초로 적용했다. 브라우저에서 짧은 대사→긴 대사→짧은 대사의 중간 높이 변화와 전환 후 인라인 높이·잔여 애니메이션 없음, 모션 감소 시 즉시 반영을 확인했다.

안내 목표 고정·다른 Prism 조작 시 EVE 힌트·상하 출구 가시성을 반영했다. 새 목표 유지 회귀 테스트를 포함한 37개 테스트 통과, 브라우저에서 다른 Prism 4회 회전 시 ◇ 유지와 EVE 힌트, 안내를 따른 11회 조작 완료 및 출구 소멸을 확인했다.

크리스탈 선명도·표시 보정: 본체 필터는 고정하고 다음 조작 발광·점등 상태를 유지하며 마우스 hover 발광은 제거했다. 출구를 본체 위에서 합성해 선명도를 회복하고 전환 레이어를 유지한다. 출구 이미지와 참조를 삭제하고 CSS 선·원으로 교체했으며 회전은 좌표만 보간한다. 1908×925·1366×768에서 회전 중 원 크기 고정, hover 전후 동일 상태, 다음 Prism 발광, 모션 감소, 직접 완료·출구 소멸을 확인했다. 개화 전후 확대 프레임에서 하단 결정·받침의 픽셀 안정성을 비교했다. 37개 테스트 통과. 사용자 환경에서 최종 시각 확인은 남아 있다.

파일명 정리: 모바일 MAP Controller는 map-mobile로 변경하고 코스터 레일 SVG 3개의 hd 접미사를 제거했다. 참조 경로를 갱신했으며 이름 변경 전후 파일 해시가 동일하다. 미사용 루나 구버전 PNG 2개를 삭제했다. 테스트 36개, 루나 출구·코스터 Guide/Play 자산 로딩과 390px 모바일 MAP 패널 펼치기를 확인했다. 루나 완료 시 출구 회전은 지연 없이 0.36초, 투명도 소멸만 1초 뒤 0.8초로 분리했다.

루나 게임 파일·클래스·번역 키·보드 상수의 sample 명칭을 game으로 정리하고 진입 경로와 테스트 참조를 갱신했다. CSS는 기존 style.css와 동일한 2칸 들여쓰기·속성별 줄바꿈으로 정리했으며 선언값은 유지했다. 자동 테스트 36개와 1908×925에서 직접 퍼즐 완료·개화 Preview, 390×844에서 초기 진입을 확인했다.

완료 연출 순서: 0~1초 밝기 상승 후, 1초 시점부터 Lotus 개화·빛줄기와 출구 소멸·문구 등장을 함께 시작한다. 개화는 2초, 빛줄기·출구 소멸과 문구 등장은 함께 0.8초 동안 진행한다.

Lotus 개화 테스트: `index.html?facility=luna&mission-preview=completed`를 추가했다. 36개 자동 테스트에서 실제 최종 회전 완료와 Preview 저장 격리를 확인했고, 1908×925 브라우저에서 3/3 대기·Pause·재개·자동 개화·완료 광선 숨김·새로고침 재진입을 확인했다.

현재 LUNA 게임은 사용자 승인에 따라 생성 원본 1672×941을 2560×1440 WebP로 변환해 복구 전후 배경에 적용했다. 신규 고해상도 원본 확보와는 구분한다. 유리·바닥·연못 테두리의 잔알갱이 표현을 줄이고, 세 군락은 초록 봉오리에서 흰 백합·살구색 꽃·보라색 꽃으로 개화 차이를 강화했다. 사용자가 승인한 광원 구체·Lotus·Prism·좌표·게임 로직은 유지 대상이다. 1908×925에서 초기·부분 복구·완료 화면과 직접 11회 조작, 완료 광선 opacity 0 및 스크립트/HTTP 오류 없음을 확인했다. 1366×768·390×844에서 초기 배경 로딩과 화면을 확인했다. 현재 밤 테마의 복구 전후 배경은 사용자 시각 승인으로 확정했다. 실제 기기·저장 연동 검증은 남아 있다. 관제실과 다른 시설 배경은 변경하지 않았다. 낮 테마는 후속 제작이며 아직 구현하지 않았다. 같은 구도에서 온실 밖 달을 해로 바꾸고, 사용자 참고 이미지의 밝고 맑은 분위기를 따르는 기준은 project.md에 기록했다.

 이 문서의 완료 목록은 재제작 대상이 아니다. 먼저 프로젝트 작업 규칙 → project.md의 공통 시스템·Mission System·LUNA 규칙 → 이 문서의 다음 작업과 보류 범위 → README.md 순서로 읽고 실제 소스를 확인한다.

### LUNA에서 확정된 것과 먼저 결정할 것

Mint 생명 시설·고정 Prism의 90° 회전·Light Fragment 수집을 유지하며, 한 화면에서 세 군락을 깨운 뒤 중앙 Lotus를 완성하는 구성으로 확정했다. 군락은 순서와 관계없이 깨어나며 빛을 옮겨도 개화가 유지된다. 별도 연결 확인과 제한 시간은 두지 않는다. NOVA의 STEP 교체와 기계 UI는 이식하지 않는다.

`luna-game.html`에 Prism 4개·군락 3곳·Lotus의 독립 퍼즐을 구현했다. 전체 화면 온실 배경·낮은 석재 받침·결정 4방향 시트·서로 다른 세 군락과 Lotus의 2상태 시트를 적용했다. EVE 원본은 유지한다. 세 조각 이전 Lotus 연결은 완료되지 않으며 완료 후 회전 잠금과 Pause 안의 다시 시작을 제공한다. MAP → 루나 관제실 → 정원 게임 진입과 관제실·MAP 복귀를 연결했다. 현재 게임은 독립 게임을 Dialog 안에서 열며 탐험 저장·완료 기록은 연결하지 않았다.

재현: 초기 A 두 번 → B 한 번으로 첫 군락, B 두 번 → C 한 번으로 두 번째, C 두 번 → D 한 번으로 세 번째, D 두 번으로 Lotus 완료. 다른 순서도 가능하다. 새로고침은 게임만 초기 상태로 돌아오며 실제 진행 기록을 읽거나 변경하지 않는다. 언어 설정만 기존 공통 저장을 사용한다.

검증: 빛의 256가지 회전 조합·여섯 수집 순서·Lotus 조건·중복 회수·완료 잠금·재시작과 기존 회귀 테스트를 합쳐 22개 통과했다. 브라우저에서 PC·390×844의 세 군락부터 Lotus까지, 키보드·한영 전환·Guide/Pause·초기화를 확인했다. 개화 도중 Pause에서 계산된 불투명도가 유지되고 재개 후 개화가 이어지는 것을 확인했다. 1280×480에서도 페이지 가로 넘침이 없었다. 작은 화면은 가로 이동 방식의 임시 대응이며 최종 모바일 HUD가 아니다. 전체 LUNA의 실제 저장·복귀 흐름과 실제 기기 검증은 미수행이다.

관제실 검수 주소: `index.html?facility=luna&mission-preview=control-room` (localhost 전용 기존 저장 격리 Preview, NOVA 완료·LUNA 개방 상태). 일반 진입은 기존 시설 해금 조건을 유지한다. 관제실에서 게임으로 들어가고 돌아오기, 뒤로가기로 MAP 복귀 후 LUNA 재진입, 한영 동기화, 1366×768·390×844 배치와 모바일 진입·복귀를 확인했다. 코스터 관제실의 기존 0/9·0/3과 Guide 진입도 확인했다. 새 관제실 회귀 테스트를 포함한 전체 24개 테스트가 통과했다. 실제 사용자 저장 데이터로 완료·Stamp까지 이어지는 흐름은 이번 범위가 아니다.

관제실 공통·루나 전용 배경 `::before` 색상 레이어를 제거했다. 기존 `::after` Box Shadow와 UI 패널은 유지한다. 이후 시설에도 배경 색상 레이어를 추가하지 않는 기준을 project.md에 반영했다.

타이틀–EVE 간격의 루나 전용 예외를 제거해 관제실 공통 여백으로 통일했다. 오른쪽 상태·목표 묶음을 콘텐츠 영역 하단에 정렬하고 데스크톱 Preview에서 배경 노출과 시작 버튼 비겹침을 확인했다. 모바일은 기존 한 열 CSS 흐름을 유지하며 이번 실제 화면 검수에는 포함하지 않았다.

관제실 배경은 원본 v1으로 복원했다. 루나 시작 버튼 최대 폭을 40rem에서 34rem으로 줄이고 패딩과 최소 높이를 조정했다. 현재 데스크톱 Preview에서 중앙 꽃 문양과 깊이 있는 테이블을 확인했으며 버튼은 앞쪽 받침 일부와 겹친다. 다른 화면 비율·모바일의 추가 검수는 남아 있다.

MAP·관제실 패널은 균일한 청회색 75%·Blur 없음으로 정리했다. 내부 Glow와 미션 패널의 잔여 배경, 모바일 미션 펼침 배경 덮어쓰기를 제거했다. MAP·코스터·루나 데스크톱과 390px MAP·루나를 확인했다. MAP 헤더·미션·EVE·최근 기록과 루나 패널의 계산 스타일을 확인했다. 전체 브라우저·모든 화면 비율과 실제 기기 검수는 미수행이다.

루나 시작 버튼을 CSS 유리 표면·금빛 이중 테두리·느린 달빛 반사로 변경했다. 데스크톱 Preview 외형과 실제 게임 진입·관제실 복귀·시작 버튼 포커스 복원을 확인했다. 브라우저 계산 스타일 조회는 selector timeout으로 실패했으며 모션 감소·완료 시 정지는 정적 규칙으로 확인했다. 이번 버튼 변경의 모바일·모션 감소 실제 화면 검수는 남아 있다.

게임 공통 UX: NOVA·LUNA가 `mission-pause.js`의 동일한 Pause 마크업·번역·버튼 순서를 사용한다. LUNA의 별도 EN·상단 관제실 복귀 바·하단 다시 체험하기를 제거하고 최초 Guide와 Pause 내 재시작·관제실·MAP 복귀를 연결했다. 가이드 복귀 시 회전 유지, 재시작 초기화, Escape Pause, MAP 복귀, 모바일 관제실 복귀 후 시작 버튼 포커스를 실제 브라우저에서 확인했다. NOVA Guide → Play → Pause와 390×844 LUNA Pause 외형을 확인했고 회귀 테스트 24개·변경 JS 문법·diff 검사를 통과했다. 이번 변경 후 전체 개화 완료·영어·낮은 높이·실제 저장 흐름은 재검수하지 않았다. 실제 완료 결과·Checkpoint·Stamp 연결은 다음 범위다.

시각 개선 비교 기준은 `c683c8a`다. 이후 게임 전용 배경·식재·Prism·Lotus 자산, 전체 화면 배치, 상하단 HUD, 군락/Lotus 진행 구분과 순차 빛·완료 연출을 개선했다. 관제실과 NOVA 코드는 이번 시각 개선에서 변경하지 않았다.

이전 V2 검증: 회귀 테스트 24개·변경 JS 문법·CSS 괄호·자산 경로·실제 RGBA·diff 검사 통과. 브라우저에서 PC 초기→세 군락→Lotus 완료·회전 잠금·Pause 재개, 관제실 진입/복귀와 한국어·영어를 확인했다. 1920×1080·1366×768·2560×1080·390×844·1280×480 화면을 확인했다. 작은 화면/낮은 높이는 스크롤 가능한 보드 대응이며 최종 모바일 HUD가 아니다. 개화 중 Pause의 계산 스타일/애니메이션 시간 조회는 도구 timeout으로 미확인, OS 모션 감소·실제 기기·실제 완료 저장 흐름은 미수행이다.

이전 V3: 빈 석재 광장을 식재·좁은 돌길·작은 연못으로 바꾸고, 군락마다 주변 화단이 복구되는 배경 레이어를 연결했다. Lotus 완료에서는 밝고 산뜻한 전체 정원으로 전환한다. Prism은 사용자가 선호한 기존 가늘고 비대칭인 결정과 아이보리 받침을 다시 사용한다. 신규 광학 결정·회색 받침과 어두운 복구 배경은 적용하지 않는다.

V3 검증: 회귀 테스트 24개, 변경 JS 문법·PNG 무결성·CSS 괄호·diff 검사 통과. 1920×1080에서 초기→부분 개화→세 군락→Lotus 완료·회전 잠금·Pause·재시작 초기화를 확인했다. 1366×768 초기 배치와 390×844 HUD·스크롤 보드·Pause 접근을 확인했다. 캡처 자체의 흐림으로 미세 선명도 검수는 제한됐다. V3에서 영어·일반 진입 저장·실제 기기·OS 모션 감소·개화 중 Pause의 계산 스타일 검수는 수행하지 않았다. 앞선 버전의 검증과 구분한다.

시설별 Preview는 `index.html?facility=coaster/luna&mission-preview=단계`를 사용한다. 코스터 8개·루나 4개이며 목록은 project.md의 화면 검수용 진입을 기준으로 한다. 예전 주소 호환 처리와 일시정지 직접 진입을 제거했다. 루나는 관제실·Guide·게임 시작과 개화 연출(`completed`)을 제공한다. 군락별 복구·Lotus 대기·조건 부족의 개별 직접 진입은 제공하지 않는다. 실제 게임의 복구·완료 동작은 유지한다. 게임 안의 공통 Pause는 유지한다. `testing` 이름은 최종점검으로 통일하며 다른 시설의 최종점검은 후속 구현 대상이다. 루나의 실제 완료 저장·복구 후 관제실도 아직 Preview에 포함하지 않는다.

검증: 전체 28개 테스트에서 유효 주소·제거된 주소 거부·외부 호스트·저장 격리·루나 개방·코스터 Play/Fail을 확인했다. JS 문법·diff 검사 통과. 이번 정리 후 브라우저의 전체 주소별 재검수는 수행하지 않았다.

루나 탭 이동 시 자동 Pause 팝업을 제거하고 문서 비표시와 수동 Pause를 함께 고려해 연출만 정지·재개한다. 수동 Guide/Pause와 코스터 자동 Pause는 유지한다.

Preview 실행 환경: 사용자 로컬 서버는 `http://127.0.0.1:5500/novaland/`이며 GitHub Pages `https://37daphne21.github.io/novaland/`도 동일한 쿼리를 허용하도록 공통 조건을 적용했다. 자동 테스트 31개에서 저장 격리·배포 경로·루나 직접 시작을 확인했다. 5500 브라우저에서 루나 게임 직접 시작과 코스터 카운트다운 표시를 확인했다. GitHub 업로드·배포 검증은 하지 않았다. 사용자가 보고한 실패는 최신 5500의 정식 쿼리에서 재현되지 않았으므로 원인을 확정하지 않는다.

루나 EVE는 동일 대사 재타이핑을 생략하고 최초·변경 대사만 타이핑한다.

헤더 정리: 전체 배경·테두리를 제거하고 중앙 군락 복구/진행 마커만 독립 유리 패널로 유지했다. 1440×900과 작은 화면에서 배치 확인, 전체 자동 테스트 31개 통과. 현재 사용 자산과 코드·문서를 커밋 대상으로 정리하며 미사용 이미지 후보는 로컬에 보존한다. 실제 저장 연동·GitHub 배포 검증은 후속 범위다.

코스터 헤더: 좌우 영역을 Grid 행 높이에 맞추고 오른쪽 Y축 보정을 제거했다. 이후 사용자 피드백에 따라 프레임 하단 여백을 반영한 상하 Padding으로 전체 콘텐츠를 올려 실제 패널 중앙에 맞췄다. 좌측 제목 줄 높이와 중앙 진행 표시의 상하 Padding을 별도로 정리해 세 영역을 각 프레임 면에 맞췄다. 1880×940에서 수정 화면을 확인했다. 타이머 숫자에 2.5em 고정 너비·tabular-nums를 적용해 기존 크기에 가깝게 유지한다. 1440×900에서 00:57→00:33 숫자 변경 전후 가이드 위치 유지와 작은 화면 배치를 확인했다. CSS diff 검사 통과. 영어·전체 흐름은 이번 CSS 변경 후 재검수하지 않았다.

헤더 선언 정리: meta의 불필요한 min-height: 0과 가이드 meta의 잔여 transform: none만 제거했다. 외형 유지 요청에 따라 가이드·모바일 높이에 영향을 주는 identity 최소 높이와 기존 가이드 버튼 높이는 유지했다.

헤더 픽셀 검수: 사용자 첨부 1870×91 이미지에서 우측 프레임 Y=11/79, Guide Y=25/64로 위·아래 경계 간 거리 14/15px을 확인했다. 제목 글자 Y=40~51은 좌측 프레임 Y=11/79 기준 29/28px이다. 막대 두께 차이는 Edge 동일 화면에서 이전 Padding과 현재 Padding을 비교해 재현했다. 높이 7.359375px은 같지만 Y=71.953125에서 71.46875로 바뀌면서 테두리가 7개에서 8개 픽셀 행에 걸쳐 렌더링됐다. 높이를 .4375rem(기본 7px)으로 정리했고 1880×940·1366×768에서 7px 높이와 7개 픽셀 행 범위를 확인했다. 가이드 위치는 추가 변경하지 않았다. 전체 회귀 테스트 32개와 diff 검사를 통과했다. 이번 수정의 실제 화면 검수는 Edge 데스크톱 두 크기이며 모바일·영어·전체 저장 흐름 재검수는 수행하지 않았다.

현재 LUNA V6: 중앙 연못·연꽃 통합 배경을 유지하면서 Lotus 높이를 조금 조정했다. Lotus만 아래로 내리던 20px 보정을 제거하고 출구·광선·◇를 같은 보드 좌표로 맞췄다. 전체 점선·전체 Prism 맥동을 제거했다. 입구 방향 조건과 두 연결면 미리보기를 제거했다. 어느 방향에서 들어오는 빛이든 회전으로 지정한 출구 하나로 보낸다. 받침 표시·한영 Guide/EVE·접근성 이름도 단일 출구에 맞췄다. 유효한 Prism·꽃·조건을 채운 Lotus는 실선이고, 허공·세 조각 전 Lotus는 점선이다. 허공 방향선은 115에서 210 보드 단위로 늘렸다. 큰 원형 포커스 테두리는 결정 윤곽의 부드러운 빛으로 대체했다. 현재 회전 상태로부터 다음 미복구 목표에 도달하는 조작을 계산해 Prism 하나만 맥동시키고 해당 목표의 실제 광선 종점에 ◇ 하나만 고정 표시하며 수직 부유 모션은 없앴다. 이미 켜진 Prism은 빛길이 끊겨도 일정한 발광을 유지하며 재시작에서 초기화한다. 완료 후 ◇와 맥동은 종료한다. 원본 고해상도 확보는 여전히 남아 있다.

V6 검증: 회귀 테스트 35개 통과. 256가지 회전에서 수광 Prism마다 지정 방향의 출사선 하나만 존재하는지와 순환 감지·여섯 수집 순서·완료 조건을 검사했다. 1908×921 실제 index에서 11번 직접 회전해 세 군락과 Lotus를 완료했고 되돌림·조기 Lotus·키보드·Guide·Pause·재시작을 확인했다. 모든 256가지 초기 회전에서 안내를 따라 완료할 수 있는지와 발광 이력 유지·재시작 초기화를 검사했다. 5500 index Preview의 1908×921에서 표시된 Prism만 따라 직접 완료했고 단계마다 ◇ 하나·맥동 하나·점선→실선 전환 및 마지막 수평선을 확인했다. 1366×768·390×844에서 키보드·Guide/Pause·재시작을 확인했다. Pause의 실행 애니메이션은 0개, 모션 감소 설정으로 새로 진입한 화면도 0개이며 정적 발광을 유지한다. 모션 설정을 바꾼 직후의 첫 조회는 이전 프레임을 반환해 별도 컨텍스트로 재확인했다. 1908×921·390×844에서 세 군락과 Lotus의 ◇ 중심이 실제 광선 종점과 0.03px 미만 오차로 일치하며 안내를 따라 완료되는 것을 확인했다. 추가 1908px 검수에서 조기 Lotus 점선·완료 실선·출구 중심 오차 0·원형 테두리 없음·모션 감소를 확인했다. 실제 기기·저장 연동·배포는 미검수다. 사용자가 게임 동작과 자연스러움을 승인했다. 1908×921에서 개화 후 광선과 네 출구의 opacity가 모두 0이 되는 것을 확인했다. Lotus 개화 후 빛줄기·출구 표시를 숨기는 마무리를 포함해 배경 품질 재작업 전 비교 기준으로 저장한다. 다음 작업은 기존 배치와 게임 동작을 유지한 배경 질감 개선 및 더 크고 화려한 Lotus 개화다.

배경 재작업 비교 기준은 `327df46`에 저장했다. 이후 V7 복구 전후 배경으로 바닥의 엉킨 무늬를 정리하고 중앙 Lotus를 크게 개화하도록 변경했다. 관제실과 이어지는 온실 재질을 기준으로 생성했으며 게임 보드 좌표와 단일 출구 로직은 유지했다. 크리스탈 발광에는 하단 마스크를 적용하고 금색 발 아래 접촉 그림자를 좁고 진하게 조정했다. 실제 자산은 1672×941이며 고해상도 원본 확보로 간주하지 않는다. 5500 index Preview 1908×921·1366×768·390×844에서 직접 11회 조작하여 완료했고 콘솔 오류 없이 완료 광선·출구 opacity 0을 확인했다. 데스크톱 시작·완료 화면을 비교했으며 실제 기기와 저장 연동은 이번 범위에서 검수하지 않았다. V7 게임 자산과 접촉 그림자 변경을 이번 관제실·이미지 정리와 함께 저장한다. 게임 배경 추가 개선은 후속 범위다.

최종 사용자 시각 승인은 남아 있으며 완성으로 간주하지 않는다. 실제 기기·일반 진입의 완료 저장·배포는 미검증이다. 작은 화면은 가로 이동, 낮은 화면은 세로 스크롤을 사용하는 기존 임시 대응이며 최종 모바일 HUD가 아니다. 사용자 요청에 따라 배경 품질 재작업 전 현재 게임을 비교 기준으로 커밋한다.

1. 관제실과 한 화면 게임의 사용자 검토 후 난이도·시각 품질을 조정한다.
2. 실제 공통화 가능한 수명주기와 시설별 게임 Module을 연결한다.
3. 최초 Guide와 공통 Pause 연결 이후, 완료 결과 UI와 실제 저장 수명주기를 연결한다. 게임 내부의 시설별 UI는 별도 제작한다.
