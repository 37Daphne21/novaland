...
1. 완료·보상 흐름과 결과 화면: Play 위 Pause·Fail·Complete 구성, 최종 점검 연결, 최초 NOVA Stamp 획득 연출
2. Guide·Play 조작 안내: 2단계 설명, 조각 반복 사용·제거 예시, 정보 중복과 시간 임박 안내
3. 확정 동작과 구현 불일치: Explorer Log 순서, 초기화 진입점, Timer·Navigation 기준 대조와 정리
4. MAP·관제실 공통 UI: 잠금 조건, Passport 아이콘, 중복 정보, 고정 환경 표시와 상태·버튼 위계
5. 모바일 MAP 복구 상태: 진행 상태별 시설 조명 반영 (모바일 전체 HUD 재설계와 구분)
6. Passport 보완: NOVA 쿠폰 상태·저장과 표지 복구 색 반영 (실제 혜택·사용 조건·발급 방식은 별도 확정 필요)
7. 문서 정합성 정리와 전체 회귀 검수: 한국어·영어, PC·모바일·Tablet·가로 화면, 저장·복귀·키보드·모션 감소

각 작업에서 확정사항은 구현에 반영하고 기획 변경이 필요한 부분은 구체안을 먼저 정한다. 문서는 작업마다 갱신하며 마지막 단계에서 통합 검수한다.
