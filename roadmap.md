...
현재 다음 제작 대상은 **LUNA LIGHT GARDEN**이다.

현재 LUNA 배경 후보는 1672×941로 새 전체 화면 원본 기준(가로 2560px 이상)에 미달한다. 추가 생성·적용은 고해상도 원본 확보 방법 확인 전 보류한다. 세 군락 Before/After 강화·유리 구체 광원 및 실제 플레이 검수는 진행 중이며 완료로 간주하지 않는다. 기존 MAP·관제실·게임 배경의 고해상도 재제작은 후속 작업으로 남기고 현재 자산을 일괄 교체하지 않는다.
...
`luna-sample.html`에 Prism 4개·군락 3곳·Lotus의 독립 퍼즐을 구현했다. 전체 화면 온실 배경·낮은 석재 받침·결정 4방향 시트·서로 다른 세 군락과 Lotus의 2상태 시트를 적용했다. EVE 원본은 유지한다. 세 조각 이전 Lotus 연결은 완료되지 않으며 완료 후 회전 잠금과 Pause 안의 다시 시작을 제공한다. MAP → 루나 관제실 → 정원 게임 진입과 관제실·MAP 복귀를 연결했다. 현재 게임은 독립 샘플을 Dialog 안에서 열며 탐험 저장·완료 기록은 연결하지 않았다.

재현: 초기 A 두 번 → B 한 번으로 첫 군락, B 두 번 → C 한 번으로 두 번째, C 두 번 → D 한 번으로 세 번째, D 두 번으로 Lotus 완료. 다른 순서도 가능하다. 새로고침은 샘플만 초기 상태로 돌아오며 실제 진행 기록을 읽거나 변경하지 않는다. 언어 설정만 기존 공통 저장을 사용한다.
...
시설별 Preview는 `index.html?facility=coaster/luna&mission-preview=단계`를 사용한다. 코스터 8개·루나 3개이며 목록은 project.md의 화면 검수용 진입을 기준으로 한다. 예전 주소 호환 처리와 일시정지 직접 진입을 제거했다. 루나는 관제실·Guide·게임 시작만 남기고 군락별 복구·Lotus 대기·조건 부족·완료 상태의 테스트 진입을 제거했다. 실제 게임의 복구·완료 동작은 유지한다. 게임 안의 공통 Pause는 유지한다. `testing` 이름은 최종점검으로 통일하며 다른 시설의 최종점검은 후속 구현 대상이다. 루나의 실제 완료 저장·복구 후 관제실도 아직 Preview에 포함하지 않는다.
